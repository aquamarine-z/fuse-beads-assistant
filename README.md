...
- 关闭整个网页或标签页后，不会保留旧图片
- 图片不再直接写入 `sessionStorage`，因此修改板子尺寸时不会再因为大图数据触发配额报错
- 大图导出页不会读取旧的“大图图纸缓存”
- 大图导出页只会在“原图纸页面仍然打开并且当前有图”的情况下拿到一次性同步数据
- 大图导出页收到的是轻量配置与原图数据，然后在当前页用同一套算法重新生成图纸
- 大图导出页会把这份轻量配置写入当前导出标签页的 `sessionStorage`
